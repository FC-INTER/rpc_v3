...
list_all_collection_1_svc(void *argp, struct svc_req *rqstp)
{
    ...
    for (int i=0 ; i<database->collections.nb_different_collection ; i++){
...
add_cloth_to_collection_1_svc(article *argp, struct svc_req *rqstp)
{
	...
	printf("---------------------------------------------------\n");
    printf("Debut ajour d'article\n");
    int nb_article = database->articles.nb_different_article;
    database->articles.article[nb_article]=*argp;
    printf("    ID : %d\n",database->articles.article[nb_article].id_article);
    printf("    Nom : %s\n",database->articles.article[nb_article].nom);
    printf("    Prix : %d\n",database->articles.article[nb_article].prix_location);
    printf("    Point de livraison : %d \n",database->articles.article[nb_article].pt_livraison);
    printf("    Stocks : %d\n",database->articles.article[nb_article].stock);
    printf("    Credit : %d\n",database->articles.article[nb_article].credit);
    //assigne la collection à l'article grace à l'id envoyé
	for (int i =0;i<database->collections.nb_different_collection;i++){
		if(database->articles.article[nb_article].collection_reference.id_collection == database->collections.collection[i].id_collection){
			database->articles.article[nb_article].collection_reference=database->collections.collection[i];
		} 
	}
    printf("Collection : %s\n",database->articles.article[nb_article].collection_reference.nom_collection);
    database->articles.nb_different_article++; 
    printf("Fin ajour d'article\n");
...
remove_cloth_to_collection_1_svc(article *argp, struct svc_req *rqstp)
{
	...
	for (int i=id_removable_article ; i<database->articles.nb_different_article ; i++){
        ...
        database->articles.article[i] = database->articles.article[i+1];
        database->articles.article[i+1] = intermediare;
    }
    printf("\n");
    for (int i = 0 ; i<database->articles.nb_different_article ; i++){
        printf("    ID article after : %d\n",database->articles.article[i].id_article);
    }
